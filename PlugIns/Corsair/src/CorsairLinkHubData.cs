...
namespace SensorReadout.CorsairPlugIn
{
    ...
    /// by the protocol: QX and RX MAX fans carry an in-fan temperature sensor (plain
    ...
    public static class LinkHubData
    {
        ...
        {
            var commandLength = command == null ? 0 : command.Length;
            var dataLength = data == null ? 0 : data.Length;
            if (outLength < 3 || commandLength + dataLength > outLength - 3)
            {
                throw new ArgumentOutOfRangeException("outLength", "The HID output report is too short for this Corsair command.");
            }
