...
class PackageControlTests(unittest.TestCase):
    ...
    def make_package(self, version="6.3.2"):
    ...
    def test_package_allow_list_version_and_hashes_are_verified(self):
        ...
        self.assertEqual("6.3.2", manifest["Version"])
    ...
    def test_failed_deploy_restores_previous_release(self):
        layout = self.layout()
        package = self.make_package()
        release = self.root / "release-6.3.2"
    ...
    def test_version_sorting_prefers_stable_release(self):
        versions = ["6.3.2-beta.2", "5.9.9", "6.3.2"]
        self.assertEqual("6.3.2", sorted(versions, key=CONTROL.version_key, reverse=True)[0])
    ...
    def test_server_update_channel_ignores_client_draft_and_prerelease_releases(self):
        releases = [
            ...
            {
                "tag_name": "server-v6.3.2",
                "draft": True,
                "assets": [{"name": "SensorReadout-Server-6.3.2.zip", "browser_download_url": "https://example.test/draft.zip"}],
            },
            {
                "tag_name": "server-v6.3.2",
                "prerelease": True,
                "assets": [{"name": "SensorReadout-Server-6.3.2.zip", "browser_download_url": "https://example.test/preview.zip"}],
            },
        ]
        self.assertIsNone(CONTROL.find_server_update(releases, "6.3.2"))
    ...
    def test_server_update_requires_exact_single_server_asset(self):
        release = {
            ...
            "assets": [
                ...
                {"name": "SensorReadout-Server-6.3.2.zip", "browser_download_url": "https://example.test/server.zip"},
            ],
        }
        version, asset = CONTROL.find_server_update([release], "6.2.0")
        self.assertEqual("6.3.2", version)
        self.assertEqual("SensorReadout-Server-6.3.2.zip", asset["name"])
        release["assets"].append(
            {"name": "sensorreadout-server-6.3.2.ZIP", "browser_download_url": "https://example.test/duplicate.zip"}
        )
    ...
    def test_online_update_downloads_verifies_and_stages_from_local_release_service(self):
        package = self.make_package()
        archive = self.root / "SensorReadout-Server-6.3.2.zip"
        with zipfile.ZipFile(archive, "w", zipfile.ZIP_DEFLATED) as output:
            for item in package.iterdir():
                output.write(item, "SensorReadout-Server-6.3.2/" + item.name)
        ...
        class ReleaseHandler(BaseHTTPRequestHandler):
            ...
            def do_GET(self):
                ...
                elif self.path == "/SensorReadout-Server-6.3.2.zip":
            ...
            [
                {
                    ...
                    "assets": [
                        {
                            "name": "SensorReadout-Server-6.3.2.zip",
                            "browser_download_url": "http://127.0.0.1:%s/SensorReadout-Server-6.3.2.zip" % port,
        ...
        self.assertEqual(["6.3.2"], staged_versions)
    ...
    def test_health_wait_retries_transient_startup_failure(self):
        layout = self.layout()
        response = {"Name": "Sensor Readout Server", "Version": "6.3.2", "ProtocolVersion": 1}
        with mock.patch.object(CONTROL, "health", side_effect=[CONTROL.ControlError("starting"), response]) as health, \
            ...
            self.assertEqual(response, CONTROL.wait_for_health(layout, "6.3.2", timeout_seconds=5))
    ...
    def test_sigterm_during_deploy_restores_previous_release(self):
        layout = self.layout()
        package = self.make_package()
        release = self.root / "release-6.3.2-interrupted"
