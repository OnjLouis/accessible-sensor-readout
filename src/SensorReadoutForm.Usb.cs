...
public sealed partial class SensorReadoutForm : Form
{
    ...
    private bool ShowSelectedReadingDetails()
    {
        ...
        using (var dialog = new Form())
        {
            ...
            var list = new ListView
            {
                ...
                MultiSelect = true,
                HideSelection = false,
                AccessibleName = "Device details",
                AccessibleDescription = "Select one or more detail rows and press Control C to copy them."
            };
            list.Columns.Add("Field", 180);
            list.Columns.Add("Value", 520);
            foreach (var pair in row.Details.OrderBy(p => UsbDetailSortIndex(p.Key)).ThenBy(p => p.Key))
            {
            ...
            copyButton.Click += delegate { CopyUsbDetails(list); };
    ...
    private void CopyUsbDetails(ListView list)
    {
        if (list == null)
        {
            return;
        ...
        var items = list.SelectedItems.Count == 0 ? list.Items.Cast<ListViewItem>().ToList() : list.SelectedItems.Cast<ListViewItem>().ToList();
        var lines = items.Select(i => i.Text + ": " + (i.SubItems.Count > 1 ? i.SubItems[1].Text : "")).ToArray();
        if (lines.Length == 0)
        {
            return;
        }
        ...
        statusLabel.Text = "Copied " + lines.Length + " USB detail row" + (lines.Length == 1 ? "" : "s") + " to clipboard.";
    }
}
