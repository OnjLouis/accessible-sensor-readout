...
- Improved: Network listening-port Details now show the owning process name and process ID where Windows exposes that information, making it easier to tell which app opened a local TCP or UDP port. This closes [issue #23](https://github.com/OnjLouis/accessible-sensor-readout/issues/23).
...
- Fixed: Preferences category checkboxes now save correctly when you restore a hidden category, and Space toggles the selected category as expected.
- Fixed: `Alt+1` through `Alt+4` in Preferences now stay within the current Hotkeys or Fan profiles tab instead of jumping back to Hotkeys from other tabs.
- Fixed: Related Windows Settings links are now more conservative, so temperature rows and other unrelated readings no longer open pages such as Display settings just because the row mentions GPU or graphics hardware.
- Improved: NVIDIA SMI driver snapshots are cached longer, reducing repeated short-lived `nvidia-smi` launches and background noise while keeping NVIDIA memory details available.
- Improved: Lenovo Laptop Support now tries additional read-only Lenovo fan-method invocation styles used by Legion-class firmware, which may expose live fan RPM where the older probe only reported method errors.
...
- Improved: Network now includes a selectable Public IP summary reading for notification area status and spoken hotkeys, combining the public-IP lookup fields returned by the online provider. Closes [issue #17](https://github.com/OnjLouis/accessible-sensor-readout/issues/17).

### 4.5.2

- Fixed: Network now shows a clear Internet connection entry while public IP lookup is loading or unavailable, instead of hiding the public IP section until the online lookup succeeds.
...
- Improved: Wi-Fi access point rows and WMI network MAC details now show OUI vendor names where the bundled MAC vendor database can identify the address prefix.
...
### 4.0.7
- Fixed: CPU and GPU temperatures, motherboard fan readings, fan hotkeys, and reports now load correctly again after 4.0.6 could prevent low-level hardware sensors from opening.
### 4.0.6
...
- Added: Performance/Overview now includes an Accessibility group with screen-reader output status, detected supported screen readers, and common Windows accessibility settings such as High Contrast, Sticky Keys, Toggle Keys, Filter Keys, Show Sounds, and audio descriptions where Windows exposes them.
- Privacy: Anonymous community stats now include Windows edition/version/build and broad accessibility feature flags, but still do not include installed programs, program usage, identifying device data, or full report rows.

### 4.0.3

- Fixed: The Portuguese manual is now a full bundled user guide instead of a short summary, matching the standard expected for shipped languages.
...
- Fixed: Community stats upload now explicitly uses modern TLS settings, preventing SSL/TLS secure channel errors on some systems.
...
- Fixed: Diagnostics no longer shows a completion dialog after opening the Reports folder, preventing an inaccessible hidden dialog if Sensor Readout is hidden while diagnostics are running. The status line now includes how long diagnostics took.
- Improved: When Sensor Readout is hidden or minimized and no alarms, fan curves, or history logging need close monitoring, automatic refresh is throttled to reduce background impact on slower laptops. Closes [issue #9](https://github.com/OnjLouis/accessible-sensor-readout/issues/9).
...
- Fixed: The Language editor now supports F3 search from the tab, remembers the last file and entry you were editing, and handles typed multi-letter navigation in the entries list.

### 3.10.1

- Fixed: Category summary rows now localize their reading and group counts instead of mixing English wording into non-English languages.
- Fixed: Italian update and install messages now use proper accented characters where needed.

### 3.10.0

- Added: Help now includes an initial setup wizard for show/hide and speak-tray hotkeys, plus a hints and tips browser that can optionally appear at startup.
- Added: Preferences now includes a Delete all settings command for starting fresh or cleaning a copy before sharing it. Logs and reports are kept.
- Fixed: Total space and used space readings can now be added to notification area status and spoken hotkeys. Closes [issue #7](https://github.com/OnjLouis/accessible-sensor-readout/issues/7).

### 3.9.2

- Improved: Bundled non-English preferences, alarm presets, hotkey presets, fan profile labels, and screen reader descriptions now have broader translation coverage, including newly localized accessible descriptions.
- Fixed: Ready removable drives such as SD cards and USB flash drives now appear with storage space and available logical-disk activity readings, making them available for notification area status, spoken hotkeys, reports, and alarms. Closes [issue #6](https://github.com/OnjLouis/accessible-sensor-readout/issues/6).

### 3.9.1

- Fixed: Sensor Readout no longer waits indefinitely for slow LibreHardwareMonitor startup on systems where hardware enumeration can take a long time. Windows-backed readings can appear first, and temperature/fan rows update when LibreHardwareMonitor finishes.
